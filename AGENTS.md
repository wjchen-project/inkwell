...
_最后更新：完成 M7 外部修改检测（新增 `src/composables/useExternalWatcher.js`：`getMetadata` 轮询 + `window.focus` 触发 + handleRef 变化重启 + `externalState: 'clean'|'pending'|'orphaned'` 状态机 + pending 不重复弹窗 + 重载走 `editorStore.markSaved` 让 vditor 自动同步；新增 `src/components/editor/ExternalChangeDialog.jsx`：`NModal preset="card"` + 三按钮「保留 / 重载 / 稍后」+ Esc = later；新增 `src/composables/useSaveOverride.js`：`useDialog().warning()` 二次确认，首次确认后置 `firstOverrideConfirmed=true` 静默写入；`useEditorStore` 新增 `firstOverrideConfirmed` 字段；`useAutoSave.triggerSave` + `TitleBar.handleSave` 接入 `ensureOverrideConfirmed()` 共享同一份标记；`TitleBar` 升级外部状态徽标 clean 不渲染 / pending 橙色 / orphaned 红色（占位）+ orphaned 下「保存」禁用）。错误处理：`getMetadata` 抛错静默 + `console.debug`（M8 接管）。验证：`npm run format` / `npm run lint` / `npm run build` 均通过，无新增依赖。同步更新 `AGENTS.md` §3 目录结构与 §6 路线图。后续：M8 外部异常处理（orphaned 状态机实际响应 + 强制另存为）、M9 体验打磨。任意超出「占位脚手架」的功能实现都属于**较大更新**，请同步更新本文件。_
