...
_最后更新：M9 起步 vditor 工具栏精简（修改 `src/components/editor/VditorEditor.jsx`：vditor 初始化选项新增 `toolbar` 数组，从默认全量中显式移除 7 项——顶层 `upload` / `record` 与「more」子菜单的 `content-theme` / `export` / `devtools` / `info` / `help`。按钮名以 `node_modules/vditor/dist/index.js:14580` 默认数组与 `dist/js/i18n/zh_CN.js` 中文文案为准。Phase 2 §4.2 原决议使用默认全量，本调整在保留常用编辑能力的前提下，按离线 Markdown 写作场景剔除「上传 / 录音 / 导出 / 主题预览 / 开发者工具 / 关于 / 帮助」。无新增依赖；不动 store / 路由 / 样式 / 主题）。验证：`npm run format` / `npm run lint` / `npm run build` 均通过。同步更新 `AGENTS.md` §6 路线图。后续：M9 体验打磨剩余子项（vditor 工具栏集成「另存为 / 设置」自定义按钮、键盘可达性、视觉一致性等，详见 `docs/05-m9-polish.md`）。任意超出「占位脚手架」的功能实现都属于**较大更新**，请同步更新本文件。_
