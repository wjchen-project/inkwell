...
└── src/
    ...
    │   └── editor/         #   - 编辑器专属（M2+ 填充）
    │       ├── TitleBar.jsx        #   - 顶部标题栏（dirty 圆点 / 外部状态徽标 / 保存 / 另存为 / 设置 / 返回，M2+M3+M5+M6+M7）
    │       ├── VditorEditor.jsx    #   - vditor 包装（M2+）
    │       ├── SettingsDrawer.jsx  #   - 设置抽屉（M5，主题 / 自动保存 / 外部修改检测 / 关于）
    │       └── ExternalChangeDialog.jsx  #   - 外部修改检测对话框（保留 / 重载 / 稍后，M7）
    ├── composables/        # 组合式函数（M3+ 填充）
    │   ├── useFileSystem.js        #   - File System Access API 封装（M2+）
    │   ├── useAutoSave.js          #   - 自动保存（M3，pending 状态二次确认 M7）
    │   ├── useUnsavedGuard.js      #   - 关闭 + 路由拦截（M4）
    │   ├── useSaveAs.js            #   - 另存为入口（包装 saveAsFile + updateFileHandle + markSaved，M6）
    │   ├── useExternalWatcher.js   #   - 外部修改轮询（lastModified 轮询 + focus 触发 + 三选项，M7）
    │   ├── useSaveOverride.js      #   - 「保留外部修改」写入前二次确认（NModal，useAutoSave/TitleBar 共享，M7）
...
- [x] 实现 Markdown 编辑器 UI 增强：自动保存（M3）、关闭拦截（M4）、主题切换 + 设置抽屉（M5）、另存为（M6）、外部修改检测（M7）、外部异常处理（M8）、体验打磨（M9）
  ...
  - M6 已完成：新增 `src/composables/useSaveAs.js`（`useFileSystem.saveAsFile` + `editorStore.updateFileHandle` + `markSaved` + 成功 toast + `isSavingAs` 防双击；用户取消 `AbortError → null` 静默不报错；写入失败 `useFileSystem` 内部已 toast 状态保留）；`TitleBar` 在「保存」与「设置」之间插入「另存为」NButton（`size=small` `quaternary` `aria-label="另存为"` `loading={isSavingAs.value}`，常驻可用无 `canSaveAs` 禁用——与主流编辑器一致允许从非 dirty 状态出副本）；`EditorView` 新增 `Ctrl/Cmd+Shift+S` `keydown` 监听（`onMounted` 注册 `onBeforeUnmount` 清理，闭包内复用同一 `handleSaveAs` 函数引用，避免与「另存为」按钮逻辑漂移），匹配 `(ctrlKey||metaKey) && shiftKey && (key==='S'||key==='s')` + `preventDefault()`；不动 `useEditorStore` / `useFileSystem`（M2 的 `saveAsFile` 已实现），本里程碑仅是组合层 + UI 接入；无新增依赖。同步更新 `AGENTS.md` §3 目录结构与 §6 路线图。任意超出「占位脚手架」的功能实现都属于**较大更新**，请同步更新本文件。
  - M7 已完成：新增 `src/composables/useExternalWatcher.js`（`getMetadata` 轮询 + `window.focus` 触发 + handleRef 变化重启 + `externalState: 'clean'|'pending'|'orphaned'` 状态机 + 首次 poll 仅建立 baseline 不触发动作 + pending 状态下不再弹窗以免重复打扰，§9 #13 + 重载路径 `editorStore.markSaved({content, lastModified})` 让 VditorEditor 现有 `watch(value)` 自动 `vditor.setValue(newContent, true)`），返回 `{ externalState, showDialog, handleDialogResolve, startWatch, stopWatch, checkNow }`；新增 `src/components/editor/ExternalChangeDialog.jsx`（`NModal preset="card"` + 三按钮「保留我的编辑 / 重新加载外部 / 稍后处理」 + Esc/mask 关闭 = later + `aria-label`）；新增 `src/composables/useSaveOverride.js`（`useDialog().warning()` 弹「外部已被修改，继续保存将覆盖外部内容？」二次确认，首次确认后置 `firstOverrideConfirmed=true` 静默写入，§9 #10）；`useEditorStore` 新增 `firstOverrideConfirmed` 字段（`loadFromFile` / `reset` 时重置，不持久化）；`useAutoSave.triggerSave` 在重试循环前调 `ensureOverrideConfirmed()`（用户取消则早退，dirty 保持）；`TitleBar.handleSave` 同样接入（手动保存 / 自动保存共享同一份 `firstOverrideConfirmed` 标记）；`TitleBar` 升级外部状态 NTag：clean 不渲染 / pending 橙色「外部已修改」/ orphaned 红色「文件不可用」（M8 占位）+ 「保存」按钮在 orphaned 下禁用；`EditorView` 调用 `useExternalWatcher(toRef(editorStore, 'fileHandle'))` 并渲染 `<ExternalChangeDialog>`，`onResolve` 走 watcher 的 `handleDialogResolve`。错误处理：`getMetadata` 抛错静默 + `console.debug`（M7 §4.2 决议，M8 接管 NotFound/Security → orphaned）。**baseline（`useEditorStore.lastExternalModified`）由 `loadFromFile` / `markSaved` 在 store 中维护；所有成功写入路径（自动保存 / 手动保存 / 另存为 / 外部重载）通过 `markSaved({ content, lastModified })` 推进 baseline——详见下方 M7 收尾修复子项。** 无新增依赖。同步更新 `AGENTS.md` §3 目录结构与 §6 路线图。任意超出「占位脚手架」的功能实现都属于**较大更新**，请同步更新本文件。
  - **M7 收尾修复：外部修改检测 baseline 自我写入识别**。原实现存在 bug：watcher 把「应用自身的 auto-save / 手动保存写入」误判为「外部修改」并触发 `ExternalChangeDialog`，复现路径 = auto-save 5s 触发写盘 → 文件 `lastModified` 推进 → 10s 后 poll 检测到 mtime 变化且 `dirty=true`（用户接着输入）→ 弹窗。根因：baseline (`lastExternalModified`) 是 watcher 闭包内的局部变量，**只在 poll 检出变化时推进；保存路径从不更新它**。`useEditorStore` 早就声明了 `lastExternalModified` ref 字段（M1 引入后一直作 dead code），但 watcher 用的是另一份局部变量，等于两份并行真理。修复走「单一真理源」：① `useExternalWatcher.js` 删除局部 `let lastExternalModified`，改为读写 `toRef(editorStore, 'lastExternalModified')`，并把 `reloadFromHandle` 改成 `markSaved({ content, lastModified: file.lastModified })` 把「刚读到的文件」baseline 推进；`startWatch` 不再重置 baseline（数据流自维护），仅保留 `editorStore.lastExternalModified = null` 兜底（handle 变 null 时清零，避免脏数值飘到下一个文件）；`checkNow` 仍对 `null` 兜底——任意路径漏传 `lastModified` 时首轮 poll 自然落基线；② `useFileSystem.js` `openFile` 返回 `{ handle, content, name, lastModified }`（file 已经在手，零额外 IO）；`saveFile` / `saveAsFile` 在 `writable.close()` 后 `handle.getFile()` 拿 fresh mtime，加入返回对象的 `lastModified` 字段（仅 `ok=true` 时有值，失败 / 取消为 `null`）；`saveFileWithPermission` 用 spread 透传 `lastModified`；③ `useEditorStore.js` 把已有但 dead code 的 `lastExternalModified` 字段激活：`loadFromFile({ ..., lastModified? })` 与 `markSaved({ content, lastModified? })` 各加一个可选字段，`typeof === 'number'` 才推进 baseline（不传保持现状，旧调用方零影响）；④ 调用方链路：`EntryView.openFile` → `loadFromFile({ lastModified })` 在打开瞬间建立 baseline；`useAutoSave.triggerSave`（两条分支）/ `TitleBar.handleSave` / `useSaveAs.handleSaveAs` / 外部重载 `reloadFromHandle` 全部走 `markSaved({ content, lastModified })`。`getFile()` 多走一次的成本几乎为零（内部就是 `handle.getFile()` → `file.lastModified`），却消除了「自我写入 vs 外部修改」的全部误判窗口，不再需要任何 watcher 端的「同步回调」hook。无新增依赖；不动 M7 主流程的状态机 / 三选项 / pending 二次确认 / 焦点监听；只改正 baseline 的存放位置与同步路径。验证：`npm run format` / `npm run lint` / `npm run build` 均通过。同步更新 `AGENTS.md` §6 路线图 M7 子项、`§7.5.1`「修改 store / composable 也算较大更新」落实与末尾「最后更新」。任意超出「占位脚手架」的功能实现都属于**较大更新**，请同步更新本文件。
  - **M9 起步：vditor 工具栏精简**。`VditorEditor.jsx` 的 vditor 初始化选项新增 `toolbar` 数组，显式从 vditor 默认全量工具栏中移除以下 7 项：① 顶部 `upload`（上传）、`record`（录音）；② 「more」子菜单中的 `content-theme`（内容主题预览）、`export`（导出）、`devtools`（开发者工具）、`info`（关于）、`help`（帮助）。保留项：emoji / headings / bold / italic / strike / link / | / list / ordered-list / check / outdent / indent / | / quote / line / code / inline-code / insert-before / insert-after / | / table / | / undo / redo / | / fullscreen / edit-mode，以及「more」子菜单中的 both / code-theme / preview（按钮名与 vditor 源码 `node_modules/vditor/dist/index.js:14580` 一致；中文字面量为 `dist/js/i18n/zh_CN.js` 翻译）。`outline`（大纲）已从工具栏移除，迁移到设置面板（详见下方 M9 收尾子项），避免双入口状态不同步。背景：vditor 默认全量工具栏是 Phase 2 §4.2 的决议，但实际体验中部分按钮对个人 / 离线 Markdown 写作场景意义有限（如 `upload` / `record` 依赖服务端中转，本项目离线使用）、与设计风格不一致（如 `devtools` 暴露实现细节、`info` / `help` 是 vditor 官方页而非本项目）。无新增依赖；不动 `useEditorStore` / Pinia / 路由。后续 M9 子项（vditor 工具栏集成「另存为 / 设置」自定义按钮、键盘可达性、视觉一致性等）按 `docs/05-m9-polish.md` 推进。同步更新 `AGENTS.md` §6 路线图与末尾「最后更新」。任意超出「占位脚手架」的功能实现都属于**较大更新**，请同步更新本文件。
  - **M9 收尾：大纲切换迁移至设置面板**。把 vditor 工具栏「more > outline」按钮移除（M9 起步已声明），把「显示文档大纲」的开关迁移到 `SettingsDrawer`（位于「主题」与「自动保存」之间）。① `src/stores/useSettingsStore.js` 新增 `outlineEnabled: false`（默认与 vditor 默认 `outline.enable` 对齐，避免老用户升级看到突然多出来的大纲面板），并补 `validateSettings` 的 boolean 校验 + `installPersistence` 的 snapshot；② `src/components/editor/VditorEditor.jsx` 新增 `outlineEnabled: Boolean` prop——初始化 vditor 时传 `outline: { enable, position: 'left' }`，`after` 回调里主动调一次 `vditor.outline.toggle(vditor, props.outlineEnabled, false)`（vditor 不会按 `options.outline.enable` 自动显隐，仅 toolbar Outline 按钮点击时切；prop 变化由 `watch(outlineEnabled)` 同步切换并写回 `vditor.options.outline.enable`，避免后续 Mode 切换读到陈旧值）；③ `src/components/editor/SettingsDrawer.jsx` 在「主题」和「自动保存」之间插入「大纲」section（`<NSwitch>` 绑 `settingsStore.outlineEnabled`），布局从原 4 节扩展为 5 节（主题 / 大纲 / 自动保存 / 外部修改检测 / 关于）；④ `src/views/EditorView.jsx` 增加 `useSettingsStore` 调用，把 `settingsStore.outlineEnabled` 作为 prop 透传给 `<VditorEditor>`。vditor 工具栏中移除 `'outline'` 后，`vditor.toolbar.elements.outline` 为 `undefined`，`outline.toggle()` 内 `vditor.toolbar.elements.outline?.firstElementChild` 通过 optional-chaining 守卫为安全 no-op（读 `node_modules/vditor/src/ts/outline/index.ts:28` 确认）。持久化复用 M1 已就位的 `useSettingsStore.installPersistence()`（`$subscribe` + 300ms 防抖 → `localStorage['md-editor-settings']`），不新增机制。无新增依赖；不动 useEditorStore / 路由 / 主题。验证：`npm run format` / `npm run lint` / `npm run build` 均通过。同步更新 `AGENTS.md` §6 路线图与末尾「最后更新」。
...
> 本节约束 AI 代理与你的协作方式，与 §7「操作建议」互补。§7 是"写什么代码"，本节是"怎么与你协作"。

### 7.5.1 核心原则：方案先行，确认后行

- **任何非平凡改动前，必须先输出方案并等待你确认**。包括但不限于：
  - 新增 / 修改 / 删除文件
  - 修改依赖（`package.json` / `package-lock.json`）
  - 修改构建配置（`vite.config.js` / `jsconfig.json` / `.oxlintrc.json` / `eslint.config.js`）
  - 修改 `AGENTS.md` / `docs/` 下的设计文档
  - 跑可能影响构建产物或 git 状态的命令（`npm install` / `git commit` / `git push` 等）
- **方案应包含**（根据任务复杂度调整）：
  - **目标**：要解决什么问题 / 达到什么效果
  - **涉及范围**：改动的文件路径清单（或「新增目录 / 不动既有文件」）
  - **关键决策**：选型 / 命名 / 与现有约定的关系
  - **验证方式**：怎么确认改对了（`format` / `lint` / `build` / 手动验证）
  - **风险 / 副作用**：是否会破坏现有功能、是否需要同步更新 `AGENTS.md`
- **方案输出后，默认停下来等你回复**。只有以下情况可以不等确认直接执行：
  - 用户明确指令「直接做 / 不要问了 / 自己决定」（视为一次性授权）
  - 用户在 `ask_user_question` 中已选定选项（视为该选项范围内的授权）
  - 纯查询类任务（读文件 / 跑 status / 查文档）——不修改任何东西
...
_最后更新：M7 外部修改检测 baseline 自我写入识别（bug fix）。

- **问题**：`useExternalWatcher` 把应用自身的 auto-save / 手动保存写入误判为「外部修改」，触发 `ExternalChangeDialog`——典型复现：auto-save 5s 写入 → 文件 `lastModified` 推进 → 10s 后 poll 检测到 mtime 变化且 `dirty=true`（用户接着输入）→ 弹窗。
- **根因**：baseline (`lastExternalModified`) 是 watcher 闭包内的局部变量，**只在 poll 检出变化时推进；保存路径从不更新它**。`useEditorStore` 早就声明了 `lastExternalModified` ref 字段（M1 引入后一直作 dead code），但 watcher 用的是另一份局部变量，等于两份并行真理。
- **修复**：走「单一真理源」——`useExternalWatcher` 删除局部 `let lastExternalModified`，改为读写 `toRef(editorStore, 'lastExternalModified')`。`useEditorStore.markSaved({ content, lastModified? })` 与 `loadFromFile({ ..., lastModified? })` 各加可选字段，存盘路径透传 `lastModified`（`useFileSystem.openFile` / `saveFile` / `saveAsFile` 在原 `getFile()` 之外再 `getFile()` 拿 fresh mtime，零额外 IO）→ `markSaved` 同步推进 baseline。`reloadFromHandle` 也走 `markSaved({ content, lastModified: file.lastModified })`，把「刚读到的文件」baseline 推进。
- **改动文件**：`src/composables/useFileSystem.js`、`src/stores/useEditorStore.js`、`src/composables/useExternalWatcher.js`、`src/composables/useAutoSave.js`、`src/composables/useSaveAs.js`、`src/components/editor/TitleBar.jsx`、`src/views/EntryView.jsx`（共 7 个，`+124 / -45`）。`useExternalWatcher` 的 `startWatch` 不再重置 baseline（数据流自维护）；`checkNow` 仍对 `null` 兜底——任意路径漏传 `lastModified` 时首轮 poll 自然落基线。handle 由有效句柄变 `null` 时清掉 baseline，避免脏 mtime 飘到下一个文件。
- **未改**：M7 主流程的状态机 / 三选项 / pending 二次确认 / 焦点监听；无新增依赖；不动 useEditorStore 公开字段集合（`lastExternalModified` 字段已存在）。
- **验证**：`npm run format` / `npm run lint` / `npm run build` 均通过。同步更新 `AGENTS.md` §6 路线图 M7 子项。任意超出「占位脚手架」的功能实现都属于**较大更新**，请同步更新本文件。_

_历史：路由改回 history 模式 + favicon 改用 `%BASE_URL%` 占位符。
