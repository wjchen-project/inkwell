...
- [x] 「另存为」按钮在 `TitleBar` 显示
- [x] 点击「另存为」→ 弹系统保存对话框
- [x] 默认文件名 = 当前 `fileName`（`untitled.md` 或实际文件名）
- [x] 文件类型过滤器显示 `.md` / `.markdown`

#### 快捷键触发

- [x] `Ctrl+Shift+S`（Linux / Windows）触发另存为
- [x] `Cmd+Shift+S`（macOS）触发另存为
- [x] 在 vditor 编辑区聚焦时也响应
- [x] 不与 vditor 内置快捷键冲突（实测）

#### 保存行为

- [x] 选择新路径 + 文件名 → 写入成功 → 弹窗关闭
- [x] 写入成功后 `useEditorStore.fileHandle` 更新为新句柄
- [x] 写入成功后 `useEditorStore.fileName` 更新为新文件名
- [x] 写入成功后 `dirty = false`，圆点消失
- [x] 写入成功后 `lastSavedContent` 更新

#### 取消行为

- [x] 用户取消保存对话框 → 不弹错误提示
- [x] 取消后 `fileHandle` / `fileName` / `dirty` 均不变

#### 异常

- [x] 写入失败（磁盘满 / IO 错误）→ toast「另存为失败：<原因>」
- [x] 失败后状态保留（脏数据未丢失）
- [x] 用户可在 dirty=true 状态下继续尝试另存为

#### 「另存为」到原路径

- [x] 「另存为」对话框中选择的路径与当前 `fileHandle` 一致 → 写入成功，效果等同于普通保存
- [x] 不会创建重复文件（同名同路径会覆盖）

### 4.2 与其他里程碑的联动

- [x] M3 自动保存通道感知新句柄：另存为后下一次自动保存写入新路径（不再弹「首次保存」对话框）
- [x] M4 关闭拦截不影响另存为流程
- [x] M7 外部修改检测对另存为后的新文件立即生效
- [x] M8 orphaned 状态下另存为仍可用（写入新路径会解除 orphaned）

### 4.3 边界

- [x] 「新建文档」（无 fileHandle）→ 「另存为」成功 → fileHandle 正常建立
- [x] 「打开文档」（有 fileHandle）→ 「另存为」到新路径 → 原文件不被修改（实测磁盘）
- [x] 内容为空时「另存为」→ 弹窗、写入空文件、不报错
...
- [x] `npm run build` 通过
- [x] `npm run format` 通过
- [x] 快捷键不触发两次（防抖或 `e.preventDefault()`）
